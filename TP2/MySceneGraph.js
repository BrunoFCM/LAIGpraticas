    ...
    parseAnimations(animationsNode) {
        ...
        for (var i = 0; i < children.length; i++) {
            ...
            for (var j = 0; j < grandChildren.length; j++) {
                ...
                //Parse translate
                let translation = this.parseCoordinates3D(grandChildren[j].children[0], "translate for the instant " + instant + "of <" + animationId + ">");
                if(!Array.isArray(translation)){
                    return translation;   
                }
                let translationVec = vec3.clone(translation);

                //Parse rotate
                let rotationNode = grandChildren[j].children[1];
                if(rotationNode.nodeName != "rotate"){
                    return "unexpected keyframe parameter (instant " + instant + "): expected <rotate>, found <" + rotationNode.nodeName + ">"; 
                }
                ...
                let scalingVec = vec3.clone(scaling);
